...
class LiveSpeechToTextAcceptanceTests(unittest.TestCase):
    def test_rime_legal_phrase_transcribes_with_hugging_face_whisper(self):
        async def run_acceptance() -> str:
            hugging_face_key = os.environ["HUGGINGFACE_API_KEY"]
            whisper_model = os.getenv("HUGGINGFACE_WHISPER_MODEL", "openai/whisper-large-v3-turbo")
            ...
            async with httpx.AsyncClient(timeout=httpx.Timeout(60.0, connect=10.0)) as client:
                transcript = await client.post(
                    ...
                    content=await _synthesize_known_legal_phrase(),
                )
        ...
        async def run_acceptance() -> dict:
            ...
            try:
                ...
                async with httpx.AsyncClient(transport=transport, base_url="http://lawyerai.test") as client:
                    with patch.object(main, "whisper_transcriber", transcriber):
